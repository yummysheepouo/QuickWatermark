...
st.set_page_config(page_title="多媒體文字水印工具", layout="centered")
st.title("多媒體文字水印工具（圖片 / GIF / MP4）")

# 上傳來源檔
uploaded = st.file_uploader(
    "上傳圖片/GIF/影片 (PNG JPG JPEG GIF MP4 MOV WEBM)",
    type=["png", "jpg", "jpeg", "gif", "mp4", "mov", "webm"]
)

# 選擇水印類型（文字 或 圖像）
watermark_type = st.radio("水印類型", ["文字", "圖像"])

# 上傳字體（可選）
font_file = st.file_uploader("可選 上傳字體文件 TTF 或 OTF（不上傳使用預設）", type=["ttf", "otf"])

# 依水印類型建立對應 UI 元件
text = None
wm_image_file = None

if watermark_type == "文字":
    text = st.text_area("水印文字（支援多行）", value="Sample Watermark")
elif watermark_type == "圖像":
    wm_image_file = st.file_uploader("上傳水印圖像 (PNG 建議帶透明)", type=["png", "jpg", "jpeg", "gif"], key="wm_img")

# 共用設定
color = st.color_picker("文字顏色", "#FFFFFF")
size = st.slider("文字大小（px）", 8, 400, 48)
scale_percent = st.slider("圖像水印大小百分比（相對於被處理影像寬度）", 1, 100, 10)
opacity = st.slider("透明度", 0.0, 1.0, 0.6)
position = st.selectbox("位置", ["top-left", "top-right", "center", "bottom-left", "bottom-right"])
angle = st.slider("旋轉角度", -180, 180, 0)
tile = st.checkbox("平鋪水印（整張影像/每幀重複）", value=False)

# 新增：平鋪密度（越小越密）
tile_density = st.slider("平鋪密度（越小越密）", 0.3, 3.0, 1.5, step=0.1)

output_format = st.selectbox("下載格式（圖片/GIF 用）", ["JPEG", "PNG", "GIF"])
...
def paste_watermark_on_frame(frame_img, watermark_img, pos, tile_flag, density):
    ...
    if tile_flag:
        # 以 density 控制步距；density 越小代表步距越小、平鋪越密集
...
if uploaded:
    try:
        ...
        wm_file_for_image = wm_image_file if watermark_type == "圖像" else None

        if fmt in ("png", "jpg", "jpeg"):
            img = Image.open(uploaded)
            out = process_static_image(img, font, watermark_type, wm_file_for_image)
            st.image(out, caption="帶水印預覽", use_container_width=True)
            buf = io.BytesIO()
            ...
            buf.seek(0)
            st.download_button("下載帶水印檔案", data=buf, file_name=filename, mime=mime)

        elif fmt == "gif":
            img = Image.open(uploaded)
            buf = process_gif(img, font, watermark_type, wm_file_for_image)
            st.image(buf, caption="帶水印 GIF 預覽", use_container_width=True)
            st.download_button("下載帶水印 GIF", data=buf, file_name="watermarked.gif", mime="image/gif")

        elif fmt in ("mp4", "mov", "webm"):
            st.info("正在處理影片，視影片長度與伺服器資源需時較久...")
            buf = process_video(uploaded, font, watermark_type, wm_file_for_image)
            st.video(buf)
            buf.seek(0)
            st.download_button("下載帶水印影片", data=buf, file_name="watermarked.mp4", mime="video/mp4")
